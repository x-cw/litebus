//============================================================================
// Filename    : adapter_err_rsp.v
// Description : Always-on FAIL guard on the TNIU Slave IP (L0) interface.
//               Pass-through when intercept is clear; absorb requests and
//               return LB_RESP_FAIL while latched intercept is on.
//============================================================================
`include "adapter_ip_defs.vh"
`default_nettype none
`timescale 1ns/1ps

module adapter_err_rsp #(
    parameter ADDR_W = 32,
    parameter DATA_W = 64,
    parameter LEN_W  = 8,
    parameter ID_W   = 8,
    ...
    reg [SYNC_FF-1:0] sync;
    reg               latched;
    wire              sync_i = sync[SYNC_FF-1];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            sync <= {sync[SYNC_FF-2:0], intercept};
            if (sync_i)
                latched <= 1'b1;
            else if (!pend_rsp)
                latched <= 1'b0;
    ...
    assign intercept_on = latched;

    wire [EXT_CMD_W-1:0] wr_cmd = u_req_w_data[EXT_WD_W +: EXT_CMD_W];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (ist)
            I_IDLE: begin
                if (u_req_r_valid) begin
                    ist  <= I_RD;
                    itxn <= rd_txn;
                end else if (u_req_w_valid && wr_last) begin
    ...
    assign d_req_r_valid = latched ? 1'b0 : u_req_r_valid;
    assign d_req_w_valid = latched ? 1'b0 : u_req_w_valid;
    assign u_req_r_ready = latched ? (ist == I_IDLE) : d_req_r_ready;
    assign u_req_w_ready = latched ? 1'b1 : d_req_w_ready;

    assign u_rsp_rd_data      = latched ? {DATA_W{1'b0}} : d_rsp_rd_data;
    assign u_rsp_rd_last      = latched ? 1'b1 : d_rsp_rd_last;
    assign u_rsp_rd_resp      = latched ? `LB_RESP_FAIL : d_rsp_rd_resp;
    assign u_rsp_rd_ext_txnid = latched ? itxn : d_rsp_rd_ext_txnid;
    assign u_rsp_rd_user      = latched ? {USER_RSP_PW{1'b0}} : d_rsp_rd_user;
    assign u_rsp_rd_valid     = latched ? ((ist == I_RD) || (ist == I_AT && ineed_r))
                                        : d_rsp_rd_valid;
    assign d_rsp_rd_ready     = latched ? 1'b0 : u_rsp_rd_ready;

    assign u_rsp_wr_resp      = latched ? `LB_RESP_FAIL : d_rsp_wr_resp;
    assign u_rsp_wr_ext_txnid = latched ? itxn : d_rsp_wr_ext_txnid;
    assign u_rsp_wr_user      = latched ? {USER_RSP_PW{1'b0}} : d_rsp_wr_user;
    assign u_rsp_wr_valid     = latched ? ((ist == I_WR) || (ist == I_AT && !ineed_r))
                                        : d_rsp_wr_valid;
    assign d_rsp_wr_ready     = latched ? 1'b0 : u_rsp_wr_ready;
