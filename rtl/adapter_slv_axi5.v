//============================================================================
// Filename    : adapter_slv_axi5.v
// Description : LiteBus TNIU Slave IP (L0) -> AXI5 master (AWATOP).
//============================================================================
`include "adapter_ip_defs.vh"
`default_nettype none
`timescale 1ns/1ps

module adapter_slv_axi5 #(
    parameter ADDR_W  = 32,
    parameter DATA_W  = 64,
    parameter LEN_W   = 8,
    parameter ID_W    = 8,
    ...
    reg [ADDITION_W-1:0] add_r;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            add_r <= {ADDITION_W{1'b0}};
        else if (s_req_r_valid && s_req_r_ready)
            add_r <= req_r_addition;
    end
    assign rsp_rd_addition = ADDITION_EN && s_rsp_rd_last ? add_r : {ADDITION_W{1'b0}};
